				...
				<div class="collapse navbar-collapse"
					...
					<ul class="nav navbar-nav navbar-right">
						...
						<c:if test="${sessionScope.auth != null }">
							<li class="active"><a
								href="mypost?user=${sessionScope.auth.getId() }">Bài đăng
									của tôi</a></li>
						</c:if>
						<c:if
			...
			<div class="row">
				<div class="col-md-9" style="margin-top: 20px;">
					<form action="mypost" method="get">
						<input class="form-control" type="text" name="searchmp" value="${not empty param.searchmp?param.searchmp:''}" placeholder="Tìm kiếm: Nhập từ gì đó trong bài đăng" aria-label="Search">
				...
				<div class="col-md-9 user-profile328903">
					<c:if test="${listMyPost.size()==0 }">
						<div align="center">
							<h4>
								Chưa có bài đăng, <a href="askquestion">đăng ngay</a>
							</h4>
						</div>
					</c:if>
					<c:forEach items="${listMyPost }" var="q">
						<div class="question-type2033">
							<div class="row">
								<div class="col-md-1">
									<div class="left-user12923 left-user12923-repeat">
										<a href="#"><img src="images/${q.getImage() }" alt="image">
										</a>
										<c:choose>
										...
										<div class="ques-icon-info3293">
											...
											<div style="float: right;">
												<a href="askquestion?post=${q.getSlug() }"><i
													...
													href="deletemypost?post=${q.getSlug() }"
													style="color: red;"
													onclick="return confirm('Bạn có chắc muốn xóa bài đăng?');"><i
					...
					<nav aria-label="Page navigation">
						<ul class="pagination">
							<c:if test="${tag > 1 }">
								<li><a
									href="mypost?page=${tag - 1}"
									aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
							</c:if>
							<c:forEach begin="1" end="${endP }" var="i">
								<li class="${tag == i?"active":""}"><a
									href="mypost?page=${i}">${i}</a></li>
							</c:forEach>
							<c:if test="${tag < endP }">
								<li><a
									href="mypost?page=${tag+1}"
